...
Orientation file for future Claude sessions in this repo. Read this first.

## What this project is

Toki is a macOS menu-bar time tracker. It is a **respectful fork** of
...
upstream CLI. See [`TOKI.md`](TOKI.md) for the fork's relationship to upstream
...
- Toki additions live in **new packages** and **new `cmd/` entries**
...
cmd/tock-desktop      Wails desktop app (Toki additions live here)
...
cmd/tock-teams-auth   short-lived WKWebView subprocess for the Teams sign-in
                      ...
                      and copied into Toki.app/Contents/MacOS/ at release time
...
TOKI.md               fork relationship + license posture
```
...
- After `desktop-build`, the bundle is renamed from `tock-desktop.app` to
  `Toki.app` by the Makefile — Wails derives the bundle directory from the
...
- `lucide-react` for icons, `sonner` for toasts, `date-fns` for time
  ...
  `App.tsx` (theme state lives in `localStorage` under `toki.theme`)
...
- Persisted UI state lives in `localStorage` keys prefixed `toki.*`
  (`toki.theme`, `toki.activityView`, `toki.showScrollbars`,
  `toki.showAccount`, `toki.displayName`, `toki.email`). Use the same
...
- `verify` — launch the built app and confirm a change actually works.
  Wraps the "build + open Toki.app + drive it" loop.
