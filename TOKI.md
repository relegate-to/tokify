...
Toki is a desktop time tracking application built as a respectful fork of
...
Toki is a fork, not a rewrite. The goal is to add a desktop UI on top of
...
- The Go module path is left as `github.com/kriuchkov/tock` so that
  upstream merges apply cleanly. Toki additions live in new packages and
  new `cmd/` entries (e.g. `cmd/tock-desktop`).

## License

Toki inherits tock's license: **GPL-3.0-or-later**. See [`LICENSE`](LICENSE).
